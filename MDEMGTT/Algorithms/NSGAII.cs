...
namespace MDEMGTT.Algorithms
{
    using MOEA.ComponentModels;
    using MOEA.AlgorithmModels.Mutation;
    using MOEA.AlgorithmModels.Crossover;
    using MOEA.AlgorithmModels.PopInit;
    using Statistics;
        ...
        where P : NondominatedSortingPopulation<S>, new()
    {
        ...
        public List<List<double>> objectives { get; set; }
        ...
        public virtual void Evaluate(Population<S> pop)
        {
            ...
            else
            {
                for (int i = 0; i < solution_count; ++i)
                {
                    ...
                    {
                        if (objectives == null)
                            objectives = new List<List<double>>();
                        List<double> rank_i_objectives = new List<double>();
                        rank_i_objectives.Add(s.FindObjectiveAt(0));
                        rank_i_objectives.Add(s.FindObjectiveAt(1));
                        objectives.Add(rank_i_objectives);
                        //ModelFragmentVector.Add(m)
                    }
                }
            }
        }
